# Experiment 1) Learning Rate and Earthquake Epicenter
...
parser = argparse.ArgumentParser(description='Train the model with different learning rates',
...
class ModelExp1(nn.Module):
    def __init__(self):
        ...
        self.fc_epi = nn.Linear(64, 2) # epicenter position

    def forward(self, x1_ts, x2_static, edge_index, edge_weight, batch):
        ...
        # Apply output layers
        # Node level predicitons (Intensity Measurements)
        ims = self.fc_ims(x)
        # Graph level predictions (Epicenter)
        x_pooled = torch_geometric.nn.global_mean_pool(x, batch)
        epi = self.fc_epi(x_pooled)
        return ims, epi
...
def evaluate_model(model, dl, criterion):
    ...
    with tqdm(total=len(dl)) as pbar:
        for idx, batch in enumerate(dl):
            ...
            # forward pass
            ims, epi = model(x1_ts, x2_static, edge_index, edge_weight, pyg_batch)
            # get targets
            y = batch.y # ground truth intensity measurements
            graph_y = batch.graph_y # epi dists (2), norm_factor (1), reference lat/lon (1)
            epi_y = graph_y.reshape(dl.batch_size, -1)[:,:2] # only use the epicenter position for loss calculation
            # compute node and graph level loss
            node_loss = criterion(ims, torch.log10(y)) # node level loss (Intensity Measurements)
            graph_loss = criterion(epi, epi_y) # graph level loss (epicenter prediction)
            # overall loss
            total_node_loss += node_loss.item()
            total_graph_loss += graph_loss.item()
            pbar.set_description(f'Evaluating...')
            pbar.update(1)
    return total_node_loss, total_graph_loss


def train_model(model, model_save_path, model_name, train_dl, val_dl, epochs, lr, checkpoint_path=None):
    ...
    for epoch in range(start_epoch, stop_epoch):
        ...
        with tqdm(total=len(train_dl)) as pbar:
            for idx, batch in enumerate(train_dl):
                ...
                node_loss = criterion(ims, torch.log10(y)) # node level loss (Intensity Measurements)
                graph_loss = criterion(epi, epi_y) # graph level loss (epicenter prediction)
                loss = node_loss + graph_loss # combined loss
                # backward pass
                loss.backward()
                # optimization step
                optimizer.step()
                ...
                total_node_loss += node_loss.item()
                total_graph_loss += graph_loss.item()
                total_loss = total_node_loss + total_graph_loss
                pbar.set_description(f'Epoch {epoch}. Loss: {loss.item()} (Node-loss: {node_loss.item()}, graph-loss: {graph_loss.item()}). Avg. Loss: {total_loss/(idx+1)}')
                pbar.update(1)

        # save training losses
        train_node_losses.append(total_node_loss / len(train_dl))
        train_graph_losses.append(total_graph_loss / len(train_dl))
        train_total_losses.append(total_loss / len(train_dl))

        # Evaluation
        val_node_loss, val_graph_loss = evaluate_model(model, val_dl, criterion)
        val_total_loss = val_node_loss + val_graph_loss
        val_node_losses.append(val_node_loss / len(val_dl))
        val_graph_losses.append(val_graph_loss / len(val_dl))
        val_total_losses.append(val_total_loss / len(val_dl))
        ...
        if epoch == 0:
            with open(csv_path, 'w', newline='') as f:
                writer = csv.writer(f)
                header = ['epoch', 'train_node_loss', 'train_graph_loss', 'train_total_loss', 'val_node_loss', 'val_graph_loss', 'val_total_loss']
                # write header
                writer.writerow(header)
        # write evaluation results to csv file
        with open(csv_path, mode='a', newline='') as f:
            writer = csv.writer(f)
            row = [epoch, train_node_losses[-1], train_graph_losses[-1], train_total_losses[-1], val_node_losses[-1], val_graph_losses[-1], val_total_losses[-1]]
...
if __name__ == '__main__':
    ...
    model_exp_1 = ModelExp1().cuda()
    epochs = args.epochs
    lr = args.lr
    checkpoint = str(args.checkpoint)
    model_save_path = 'adjust/this/path'
    model_name = f'ModelExp1_lr{lr}'
    print(f'Starting Training with Learning rate: {lr} for {epochs} epochs. \n==============================================')
    model = train_model(model_exp_1, model_save_path, model_name, train_dl, val_dl, epochs, lr, checkpoint_path=checkpoint)
