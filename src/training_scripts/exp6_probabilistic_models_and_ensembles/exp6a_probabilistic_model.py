# Experiment 6a)  Probabilistic Model
...
parser.add_argument('--id', type=int, help='ID to save model (e.g. for training multiple models for an ensemble)')
...
def evaluate_model(model, dl, criterion):
    ...
    with tqdm(total=len(dl)) as pbar:
        for idx, batch in enumerate(dl):
            ...
            # forward pass
            mean, variance = model(x1_ts, x2_static, edge_index, edge_weight, pyg_batch)
            # get targets
            ...
            pbar.update(1)
    return total_node_loss, total_node_mse_loss
...
def train_model(model, model_save_path, model_name, train_dl, val_dl, epochs, lr, checkpoint_path=None):
    ...
    for epoch in range(start_epoch, stop_epoch):
        ...
        # Evaluation
        val_node_loss, val_node_mse_loss = evaluate_model(model, val_dl, criterion)
        val_node_losses.append(val_node_loss / len(val_dl))
        val_node_mse_losses.append(val_node_mse_loss / len(val_dl))

        # Plot message
        print(f'Evaluation: Train Loss (Node): {train_node_losses[-1]} | Val Loss (Node): {val_node_losses[-1]} | MSE Val Loss (Node): {val_node_mse_losses[-1]}')

        # check if this is the best model so far:
        if (val_node_loss / len(val_dl)) <= min(val_node_losses):
            print('New best validation loss. Saving model...')
            # save the model state
            out_path = pathlib.Path(model_save_path) / f'{is_finetuning}{model_name}_epoch{epoch}_val_loss_{val_node_losses[-1]}_{val_node_mse_losses[-1]}_best.pt'
            torch.save({
                'epoch': epoch,
        ...
        if (val_node_mse_loss / len(val_dl)) <= min(val_node_mse_losses):
            ...
            # save the model state
            out_path = pathlib.Path(model_save_path) / f'{is_finetuning}{model_name}_epoch{epoch}_val_loss_{val_node_losses[-1]}_{val_node_mse_losses[-1]}_best_mse.pt'
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
        ...
        if epoch == 0:
            with open(csv_path, 'w', newline='') as f:
                writer = csv.writer(f)
                header = ['epoch', 'train_node_loss', 'val_node_loss', 'val_node_mse_loss']
                # write header
                writer.writerow(header)
        # write evaluation results to csv file
        with open(csv_path, mode='a', newline='') as f:
            writer = csv.writer(f)
            row = [epoch, train_node_losses[-1], val_node_losses[-1], val_node_mse_losses[-1]]
            # write line to csv file
            writer.writerow(row)

    # After the training, save the last model state
    out_path = pathlib.Path(model_save_path) / f'{is_finetuning}{model_name}_epoch{epoch}_val_loss_{val_node_losses[-1]}_{val_node_mse_losses[-1]}_last.pt'
    torch.save({
        'epoch': epoch,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
...
if __name__ == '__main__':
    ...
    model_name = f'ModelExp6a_ID{model_id}_lr{lr}'
    print(f'Starting Training with Learning rate: {lr} for {epochs} epochs. \n==============================================')
    model = train_model(model_exp_6a, model_save_path, model_name, train_dl, val_dl, epochs, lr, checkpoint_path=checkpoint)
