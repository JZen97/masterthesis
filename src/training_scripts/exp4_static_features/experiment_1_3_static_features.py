# Experiment 1.4) Influence of Static Features
...
parser.add_argument('--model', type=str, help='which model to use (all trained on node-level targets only). (no_static_feats: no static features), (concat_static_feats: concatenate static features), (mlp_static_feats: concatenate static features passed through an MLP)')
...
class ModelExp4_no_static_feats(nn.Module):
    def __init__(self):
        super().__init__()
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
        ...
        # Graph Layers
        self.gcn1 = torch_geometric.nn.GCNConv(10048, 64, add_self_loops=True, normalize=True, bias=False)
...
class ModelExp4_concat_static_feats(nn.Module):
    def __init__(self):
        super().__init__()
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
...
class ModelExp4_mlp_static_feats(nn.Module):
    def __init__(self):
        super().__init__()
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
        ...
        self.mlp = nn.Sequential(
            self.fc1, self.relu, self.bn1, self.dropout, self.fc2, self.relu, self.bn2, self.dropout, self.fc3, self.relu
        )

        # Graph Layers
        self.gcn1 = torch_geometric.nn.GCNConv(10112, 64, add_self_loops=True, normalize=True, bias=False)
        self.gcn2 = torch_geometric.nn.GCNConv(64, 64, add_self_loops=True, normalize=True, bias=False)
    ...
    def forward(self, x1_ts, x2_static, edge_index, edge_weight, batch):
        ...
        # flatten the cnn output
        x = torch.flatten(x, start_dim=1) # out: bs x 10048
        # Pass static features through an MLP and concatenate with CNN output
        static_feats = self.mlp(x2_static)
        x = torch.concat([x, static_feats], dim=1) # out: bs x 10051
        # Apply graph layers
        x = self.relu(self.gcn1(x, edge_index=edge_index, edge_weight=edge_weight)) # out: bs x 64
        x = self.tanh(self.gcn2(x, edge_index=edge_index, edge_weight=edge_weight)) # out: bs x 64
        # Apply output layers
...
def train_model(model, model_save_path, model_name, train_dl, val_dl, epochs, lr, checkpoint_path=None):
    ...
    for epoch in range(start_epoch, stop_epoch):
        ...
        with open(csv_path, mode='a', newline='') as f:
            writer = csv.writer(f)
            row = [epoch, train_node_losses[-1], val_node_losses[-1]]
...
if __name__ == '__main__':
    ...
    # define model
    if args.model == 'no_static_feats':
        model = ModelExp4_no_static_feats().cuda()
    elif args.model == 'concat_static_feats':
        model = ModelExp4_concat_static_feats().cuda()
    elif args.model == 'mlp_static_feats':
        model = ModelExp4_mlp_static_feats().cuda()

    epochs = args.epochs
    lr = args.lr
    checkpoint = str(args.checkpoint)
    model_save_path = 'adjust/this/path'
    model_name = f'ModelExp4_{args.model}_lr{lr}'
    print(f'Starting Training with Learning rate: {lr} for {epochs} epochs. \n==============================================')
    model = train_model(model, model_save_path, model_name, train_dl, val_dl, epochs, lr, checkpoint_path=checkpoint)
