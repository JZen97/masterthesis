# Experiment 2c) Minimum Node Degree and Maximum Graph Diameter
...
#parser.add_argument('--min_node_degree', type=int, help='minimum node degree d')
parser.add_argument('--graph_format', type=str, help='whether to filter out edges according to min_node_degree_d. For filtering use "reduced", for no filtering use "complete"')
parser.add_argument('--k_hop_reachability', type=int, help='maximum graph diameter')
parser.add_argument('--min_node_degree', type=int, help='minimum node degree')
parser.add_argument('--deterministic', type=bool, help='whether to make validation sampling deterministic')
...
def train_model(model, model_save_path, model_name, train_dl, val_dl, epochs, lr, checkpoint_path=None):
    ...
    print(f'Starting training training for epochs {start_epoch} to {stop_epoch}...')
...
if __name__ == '__main__':
    ...
    k_hop_reachability = args.k_hop_reachability
    ...
    train_ds = GIDatasetPyG(meta_csv_path=meta_csv_path,
                            ...
                            k_hop_reachability=k_hop_reachability,
                            pad_ts_s=5,
                            verbose=False)
    ...
    model_name = f'ModelExp2c_min_node_degree_and_diameter__graph_{graph_format}_d{min_node_degree}_k_hop{k_hop_reachability}_lr{lr}'
    print(f'Starting Training with Learning rate: {lr} for {epochs} epochs (graph format: {graph_format}, min node degree: {min_node_degree}, diameter: {k_hop_reachability}, deterministic: {deterministic}). \n==============================================')
    model = train_model(model_exp_2c, model_save_path, model_name, train_dl, val_dl, epochs, lr, checkpoint_path=checkpoint)
