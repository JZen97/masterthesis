# Experiment 2a) Minimum Node Degree
...
parser.add_argument('--deterministic', type=bool, help='whether the training should be deterministic')
...
if __name__ == '__main__':
    ...
    distances_npy_path = 'data/GraphInstance/station_distances_km.npy'
    ...
    # Minimum Node Degree and graph format (whether to filter or not)
    min_node_degree = int(args.min_node_degree)
    graph_format = str(args.graph_format)
    ...
    model_exp_2a = ModelExp2a().cuda()
    epochs = args.epochs
    lr = args.lr
    checkpoint = str(args.checkpoint)
    model_save_path = 'adjust/this/path'
    model_name = f'ModelExp2a_{graph_format}_d{min_node_degree}_lr{lr}_deterministic_{deterministic}'
    print(f'Starting Training with Learning rate: {lr} for {epochs} epochs with graph_format={graph_format} and min_node_degree={min_node_degree} (deterministic: {deterministic}). \n==============================================')
    model = train_model(model_exp_2a, model_save_path, model_name, train_dl, val_dl, epochs, lr, checkpoint_path=checkpoint)
