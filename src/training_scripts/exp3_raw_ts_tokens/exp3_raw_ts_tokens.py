# Experiment 3) Raw Time Series Tokens
...
class ModelExp3(nn.Module):
    def __init__(self):
        ...
        # Graph Layers
        self.gcn1 = torch_geometric.nn.GCNConv(10351, 64, add_self_loops=True, normalize=True, bias=False)
    ...
    def forward(self, x1_ts, x2_static, edge_index, edge_weight):
        ...
        # Concatenate with raw time series tokens (last 100 tokens -> last second) --> 3 * 100 = 300 values
        x1_ts_flat_last_100t = torch.flatten(x1_ts[:, :, 900:], start_dim=1)
        x = torch.concat([x, x1_ts_flat_last_100t], dim=1)
...
if __name__ == '__main__':
    ...
    model_exp_3 = ModelExp3().cuda()
    epochs = args.epochs
    lr = args.lr
    checkpoint = str(args.checkpoint)
    model_save_path = 'adjust/this/path'
    model_name = f'ModelExp3_raw_ts_tokens_lr{lr}'
    print(
        f'Starting Training (ModelExp1_2_flatten_w_raw_tokens) with Learning rate: {lr} for {epochs} epochs. \n==============================================')
    model = train_model(model_exp_3, model_save_path, model_name, train_dl, val_dl, epochs, lr, checkpoint_path=checkpoint)
